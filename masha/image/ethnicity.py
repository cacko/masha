...
class EthnicityMeta(type):
    ...
    def classify(cls, image: Path, threshold=0.2) -> list[ClassifyResult]:
        return cls().getAnswer(image, threshold)


class EthnicityClassifier(ImageClassify, metaclass=EthnicityMeta):
    pass
