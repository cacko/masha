...
class StableDiffusionFlux(BaseStableDiffusion, LoadersFluxMixin):
    ...
    def get_text2img_pipeline(self, pipe_args):
        ...
        self.pipeline = FluxPipeline(model)
        ...
        nn.quantize(self.pipeline.flow, class_predicate=quantization_predicate)
        nn.quantize(self.pipeline.t5, class_predicate=quantization_predicate)
        nn.quantize(self.pipeline.clip, class_predicate=quantization_predicate)
    ...
    def get_txt2img_result(
        ...
        # Make the generator
        latent_size = to_latent_size((output_params.width, output_params.height))
        latents = self.pipeline.generate_latents(
            output_params.prompt,
            n_images=output_params.num_images_per_prompt,
            num_steps=output_params.num_inference_steps,
        ...
        del self.pipeline.t5
        del self.pipeline.clip

        # Actual denoising loop
        for x_t in tqdm(latents, total=output_params.num_inference_steps):
            mx.eval(x_t)

        # The following is not necessary but it may help in memory constrained
        # systems by reusing the memory kept by the flow transformer.
        del self.pipeline.flow
        peak_mem_generation = mx.metal.get_peak_memory() / 1024**3
        mx.metal.reset_peak_memory()

        # Decode them into images
        decoded = []
        for i in tqdm(range(0, output_params.num_images_per_prompt, 1)):
            decoded.append(self.pipeline.decode(x_t[i : i + 1], latent_size))
            mx.eval(decoded[-1])
        ...
        images = [Image.fromarray(np.array(x[i])) for i in range(len(x))]
        ...
        return (
            FluxPipelineOutput(images=images),
            output_params,
        )
